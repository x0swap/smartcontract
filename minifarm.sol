...
contract MiniFarm is BoringOwnable, BoringBatchable {
    ...
    uint256 public constant dayInTime = 86400;
    uint256[4] public periodInDays;
    uint256[4] public periodShares;
    uint256[4] public periodInTimes;
    ...
    constructor(
        ...
        for(uint256 i=0; i<4; ++i){
            periodInTimes[i] = periodInDays[i]*dayInTime;
        }
    }
    ...
    function deposit(uint256 _pid, uint256 _fpid, uint256 _amount) public {
        ...
        user.userPeriod[_fpid].withdrawTime = user.userPeriod[_fpid].depositTime + periodInTimes[_fpid];
    ...
    /// @param _amount LP token amount to withdraw.
    function withdrawAndHarvest(uint256 _pid, uint256 _fpid, uint256 _amount) public {
        FarmPeriod memory farmPeriod = updatePool(_pid, _fpid);
        UserInfo storage user = userInfo[_pid][msg.sender];
