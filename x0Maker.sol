...
contract X0Maker is Ownable {
    ...
    address public x0Bar;
    address public x0;
    address public weth;

    constructor(IUniswapV2Factory _factory, address _x0Bar, address _x0, address _weth) public {
        factory = _factory;
        x0 = _x0;
        x0Bar = _x0Bar;
        weth = _weth;
    ...
    function setX0Bar(address _x0Bar) public onlyOwner {
        x0Bar = _x0Bar;
    }
    ...
    function _toWETH(address token) internal returns (uint256) {
        if (token == x0) {
            ...
            _safeTransfer(token, x0Bar, amount);
            return 0;
        }
        if (token == weth) {
            uint amount = IERC20(token).balanceOf(address(this));
            _safeTransfer(token, factory.getPair(weth, x0), amount);
            return amount;
        }
        ...
        pair.swap(amount0Out, amount1Out, factory.getPair(weth, x0), new bytes(0));
        return amountOut;
    }

    function _toX0(uint256 amountIn) internal {
        IUniswapV2Pair pair = IUniswapV2Pair(factory.getPair(weth, x0));
        (uint reserve0, uint reserve1,) = pair.getReserves();
        address token0 = pair.token0();
        ...
        pair.swap(amount0Out, amount1Out, x0Bar, new bytes(0));
    }
