...
contract MiniChefV2 is BoringOwnable, BoringBatchable {
    ...
    event LogUpdatePool(uint256 indexed pid, uint64 lastRewardTime, uint256 lpSupply, uint256 accX0PerShare);
    ...
    function pendingX0(uint256 _pid, address _user) external view returns (uint256 pending) {
        PoolInfo memory pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accX0PerShare = pool.accX0PerShare;
        uint256 lpSupply = lpToken[_pid].balanceOf(address(this));
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            uint256 time = block.timestamp.sub(pool.lastRewardTime);
            uint256 x0Reward = time.mul(x0PerSecond).mul(pool.allocPoint) / totalAllocPoint;
            accX0PerShare = accX0PerShare.add(x0Reward.mul(ACC_X0_PRECISION) / lpSupply);
        }
        pending = int256(user.amount.mul(accX0PerShare) / ACC_X0_PRECISION).sub(user.rewardDebt).toUInt256();
    }
    ...
    function updatePool(uint256 pid) public returns (PoolInfo memory pool) {
        ...
        if (block.timestamp > pool.lastRewardTime) {
            ...
            if (lpSupply > 0) {
                uint256 time = block.timestamp.sub(pool.lastRewardTime);
                uint256 x0Reward = time.mul(x0PerSecond).mul(pool.allocPoint) / totalAllocPoint;
                pool.accX0PerShare = pool.accX0PerShare.add((x0Reward.mul(ACC_X0_PRECISION) / lpSupply).to128());
            }
            pool.lastRewardTime = block.timestamp.to64();
            poolInfo[pid] = pool;
            emit LogUpdatePool(pid, pool.lastRewardTime, lpSupply, pool.accX0PerShare);
        }
    }
    ...
    function deposit(uint256 pid, uint256 amount, address to) public {
        ...
        user.rewardDebt = user.rewardDebt.add(int256(amount.mul(pool.accX0PerShare) / ACC_X0_PRECISION));

        // Interactions
        // IRewarder _rewarder = rewarder[pid];
        // if (address(_rewarder) != address(0)) {
        //     _rewarder.onX0Reward(pid, to, to, 0, user.amount);
        // }
    ...
    function withdraw(uint256 pid, uint256 amount, address to) public {
        ...
        // Effects
        user.rewardDebt = user.rewardDebt.sub(int256(amount.mul(pool.accX0PerShare) / ACC_X0_PRECISION));
        user.amount = user.amount.sub(amount);

        // Interactions
        // IRewarder _rewarder = rewarder[pid];
        // if (address(_rewarder) != address(0)) {
        //     _rewarder.onX0Reward(pid, msg.sender, to, 0, user.amount);
        // }
    ...
    /// @param pid The index of the pool. See `poolInfo`.
    /// @param to Receiver of X0 rewards.
    function harvest(uint256 pid, address to) public {
        PoolInfo memory pool = updatePool(pid);
        UserInfo storage user = userInfo[pid][msg.sender];
        int256 accumulatedX0 = int256(user.amount.mul(pool.accX0PerShare) / ACC_X0_PRECISION);
        uint256 _pendingX0 = accumulatedX0.sub(user.rewardDebt).toUInt256();

        // Effects
        user.rewardDebt = accumulatedX0;

        // Interactions
        if (_pendingX0 != 0) {
            X0.safeTransfer(to, _pendingX0);
        }
        
        // IRewarder _rewarder = rewarder[pid];
        // if (address(_rewarder) != address(0)) {
        //     _rewarder.onX0Reward( pid, msg.sender, to, _pendingX0, user.amount);
        // }

        emit Harvest(msg.sender, pid, _pendingX0);
    }
    
    /// @notice Withdraw LP tokens from MCV2 and harvest proceeds for transaction sender to `to`.
    /// @param pid The index of the pool. See `poolInfo`.
    /// @param amount LP token amount to withdraw.
    /// @param to Receiver of the LP tokens and X0 rewards.
    function withdrawAndHarvest(uint256 pid, uint256 amount, address to) public {
        ...
        // Effects
        user.rewardDebt = accumulatedX0.sub(int256(amount.mul(pool.accX0PerShare) / ACC_X0_PRECISION));
        user.amount = user.amount.sub(amount);
    ...
    function emergencyWithdraw(uint256 pid, address to) public {
        ...
        // IRewarder _rewarder = rewarder[pid];
        // if (address(_rewarder) != address(0)) {
        //     _rewarder.onX0Reward(pid, msg.sender, to, 0, 0);
        // }
